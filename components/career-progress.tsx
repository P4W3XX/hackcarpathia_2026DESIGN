...
export const CareerProgress: React.FC<CareerProgressProps> = ({
  ...
  const calculateLevelProgress = (levelIndex: number): number => {
    ...
    const completedReqs =
      userProgress.completedRequirements[level.level]?.length || 0;
    const completedSkills =
      userProgress.acquiredSkills[level.level]?.length || 0;
                      ...
                      <ul className="space-y-2 ml-7">
                        {level.requirements.map((req, idx) => {
                          const isReqCompleted =
                            userProgress.completedRequirements[
                              level.level
                            ]?.includes(idx);
                          return (
                            <li
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2 ml-7">
                        {level.skills.map((skill, idx) => {
                          const isSkillAcquired =
                            userProgress.acquiredSkills[level.level]?.includes(
                              idx,
                            );
                          return (
                            <span
